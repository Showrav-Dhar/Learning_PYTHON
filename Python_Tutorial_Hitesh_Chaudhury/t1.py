...
def GreedySeach(source, destination):
    visited = {}
    parent = {}

    result = []
    queue = Queue()  # to maintain the serial of traversal
    ...
    start = source
    visited[start] = True
    queue.put(start)  # Put Start City in the queue.

    while not queue.empty():
        u = queue.get()  # get() – return an item from the queue.
        result.append(u)
